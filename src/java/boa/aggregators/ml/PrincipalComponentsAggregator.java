...
/**
 * A Boa aggregator for training the model using PrincipalComponents.
 * 
 * @author ankuraga
 */
@AggregatorSpec(name = "principalcomponents", formalParameters = {"string"})
public class PrincipalComponentsAggregator extends MLAggregator {
	private ArrayList<String> vector = new ArrayList<String>();
	private ArrayList<BoaTup> tuples = new ArrayList<BoaTup>();
	private String[] options;
	private int count = 0;
	private PrincipalComponents model;
	private int NumOfAttributes;
	private ArrayList<Attribute> fvAttributes;
	private Instances trainingSet;
	private boolean flag;

	public PrincipalComponentsAggregator() {
	}

	public PrincipalComponentsAggregator(final String s) {
		super(s);
	...
	public void finish() throws IOException, InterruptedException {
		try {
			this.model = new PrincipalComponents();
	...
	public void attributeCreation(BoaTup data) {
		...
		try {
			...
			this.flag = true;
			this.trainingSet = new Instances("PrincipalComponents", this.fvAttributes, 1);
