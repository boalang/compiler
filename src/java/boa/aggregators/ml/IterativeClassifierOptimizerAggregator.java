...
/**
 * A Boa aggregator for training the model using IterativeClassifierOptimizer.
 * 
 * @author ankuraga
 */
@AggregatorSpec(name = "iterativeclassifieroptimizer", formalParameters = {"string"})
public class IterativeClassifierOptimizerAggregator extends MLAggregator {
	private ArrayList<String> vector = new ArrayList<String>();
	private ArrayList<BoaTup> tuples = new ArrayList<BoaTup>();
	private String[] options;
	private int count = 0;
	private IterativeClassifierOptimizer model;
	private int NumOfAttributes;
	private ArrayList<Attribute> fvAttributes;
	private Instances trainingSet;
	private boolean flag;

	public IterativeClassifierOptimizerAggregator() {
	}

	public IterativeClassifierOptimizerAggregator(final String s) {
		super(s);
	...
	public void finish() throws IOException, InterruptedException {
		try {
			this.model = new IterativeClassifierOptimizer();
	...
	public void attributeCreation(BoaTup data) {
		...
		try {
			...
			this.flag = true;
			this.trainingSet = new Instances("IterativeClassifierOptimizer", this.fvAttributes, 1);
