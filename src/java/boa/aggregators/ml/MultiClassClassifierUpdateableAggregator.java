...
/**
 * A Boa aggregator for training the model using MultiClassClassifierUpdateable.
 * 
 * @author ankuraga
 */
@AggregatorSpec(name = "multiclassclassifierupdateable", formalParameters = {"string"})
public class MultiClassClassifierUpdateableAggregator extends MLAggregator {
	private ArrayList<String> vector = new ArrayList<String>();
	private ArrayList<BoaTup> tuples = new ArrayList<BoaTup>();
	private String[] options;
	private int count = 0;
	private MultiClassClassifierUpdateable model;
	private int NumOfAttributes;
	private ArrayList<Attribute> fvAttributes;
	private Instances trainingSet;
	private boolean flag;

	public MultiClassClassifierUpdateableAggregator() {
	}

	public MultiClassClassifierUpdateableAggregator(final String s) {
		super(s);
	...
	public void finish() throws IOException, InterruptedException {
		try {
			this.model = new MultiClassClassifierUpdateable();
	...
	public void attributeCreation(BoaTup data) {
		...
		try {
			...
			this.flag = true;
			this.trainingSet = new Instances("MultiClassClassifierUpdateable", this.fvAttributes, 1);
