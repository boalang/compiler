...
/**
 * A Boa aggregator for training the model using BayesNetGenerator.
 * 
 * @author ankuraga
 */
@AggregatorSpec(name = "bayesnetgenerator", formalParameters = {"string"})
public class BayesNetGeneratorAggregator extends MLAggregator {
	private ArrayList<String> vector = new ArrayList<String>();
	private ArrayList<BoaTup> tuples = new ArrayList<BoaTup>();
	private String[] options;
	private int count = 0;
	private BayesNetGenerator model;
	private int NumOfAttributes;
	private ArrayList<Attribute> fvAttributes;
	private Instances trainingSet;
	private boolean flag;

	public BayesNetGeneratorAggregator() {
	}

	public BayesNetGeneratorAggregator(final String s) {
		super(s);
	...
	public void finish() throws IOException, InterruptedException {
		try {
			this.model = new BayesNetGenerator();
	...
	public void attributeCreation(BoaTup data) {
		...
		try {
			...
			this.flag = true;
			this.trainingSet = new Instances("BayesNetGenerator", this.fvAttributes, 1);
