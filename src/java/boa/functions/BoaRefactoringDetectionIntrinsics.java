...
//		System.out.println("-----------------------------------------------------------\n");
	...
	private static void removeCompatibleMetodInvocationsAndArrayAccesses1(HashSet<Expression> methodInvocations,
			HashSet<Expression> arrayAccesses, String prettyPrint1, String prettyPrint2, int distance)
			throws Exception {
		HashSet<Expression> methodInvocationRemoveSet = new HashSet<Expression>();
		HashSet<Expression> arrayAccesseRemoveSet = new HashSet<Expression>();
		for (Iterator<Expression> itr1 = methodInvocations.iterator(); itr1.hasNext();) {
			...
			if (!map.isEmpty()) {
				...
				methodInvocationRemoveSet.add(bestPairs.get(0));
				arrayAccesseRemoveSet.add(bestPairs.get(1));
			}
		}
		methodInvocations.removeAll(methodInvocationRemoveSet);
		arrayAccesses.removeAll(arrayAccesseRemoveSet);
	}
	...
	private static void removeCompatibleMetodInvocationsAndArrayAccesses2(HashSet<Expression> arrayAccesses,
		...
		for (Iterator<Expression> itr1 = arrayAccesses.iterator(); itr1.hasNext();) {
			...
			if (!map.isEmpty()) {
				ArrayList<Expression> bestPairs = map.firstEntry().getValue();
				arrayAccesseRemoveSet.add(bestPairs.get(0));
				methodInvocationRemoveSet.add(bestPairs.get(1));
			}
		}
		methodInvocations.removeAll(methodInvocationRemoveSet);
		arrayAccesses.removeAll(arrayAccesseRemoveSet);
	...
	private static void removeCompatibleVariablesAndExpressions(HashSet<String> variables,
		...
		if (variables.size() <= expressions.size()) {
			removeCompatibleVariablesAndExpressions1(variables, expressions, prettyPrint1, prettyPrint2, distance);
		} else {
			removeCompatibleVariablesAndExpressions2(expressions, variables, prettyPrint2, prettyPrint1, distance);
		}
	}
	...
	private static void removeCompatibleVariablesAndExpressions2(HashSet<Expression> expressions,
		...
		HashSet<Expression> expressionRemoveSet = new HashSet<Expression>();
		HashSet<String> variableRemoveSet = new HashSet<String>();
...
//						&& syntaxAwareReplacement(s1, s2, prettyprint1, prettyprint2)
			...
			if (!map1.isEmpty()) {
				variableRemoveSet.add(map2.firstEntry().getValue());
				expressionRemoveSet.add(map1.firstEntry().getValue());
			}
		}
		expressions.removeAll(expressionRemoveSet);
		variables.removeAll(variableRemoveSet);
	}
...
//						&& syntaxAwareReplacement(s1, s2, prettyprint1, prettyprint2)
			...
			if (!map1.isEmpty()) {
				variableRemoveSet.add(map1.firstEntry().getValue());
				expressionRemoveSet.add(map2.firstEntry().getValue());
			}
		}
		expressions.removeAll(expressionRemoveSet);
		variables.removeAll(variableRemoveSet);
	...
	private static void removeCompatibleMetodInvocationsAndCreations1(HashSet<Expression> set1,
		...
		HashSet<Expression> RemoveSet1 = new HashSet<Expression>();
		HashSet<Expression> RemoveSet2 = new HashSet<Expression>();
		for (Iterator<Expression> itr1 = set1.iterator(); itr1.hasNext();) {
			...
			if (!map.isEmpty()) {
				ArrayList<Expression> bestPairs = map.firstEntry().getValue();
				RemoveSet1.add(bestPairs.get(0));
				RemoveSet2.add(bestPairs.get(1));
			}
		}
		set1.removeAll(RemoveSet1);
		set2.removeAll(RemoveSet2);
	}
	...
	@FunctionSpec(name = "compatiblestrings", returnType = "bool", formalParameters = { "string", "string", "string",
			"string" })
	...
	private static void removeCompatibleCodeElements(HashSet<String> set1, HashSet<String> set2, String prettyPrint1,
		...
		HashSet<String> RemoveSet1 = new HashSet<String>();
		HashSet<String> RemoveSet2 = new HashSet<String>();
