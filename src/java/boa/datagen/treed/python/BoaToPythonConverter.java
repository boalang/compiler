...
public class BoaToPythonConverter {
		...
		for (String imp : node.getImportsList()) {
			if (imp.startsWith("from")) {
				...
				if (v == -1) {
					...
					ast.addStatement(new PythonImportStatement(new DLTKToken(), this.makeImportASExpression(
							BoaStringIntrinsics.substring(imp, 0, v), BoaStringIntrinsics.substring(imp, v + 4))));
			}
		}
	...
	public final ASTNode visit(final Declaration node) throws Exception {
		...
		for (boa.types.Ast.Modifier st : node.getModifiersList()) {
			ASTNode chast = visit(st);
			if (chast != null)
				decorators.add((Decorator) chast);
		}
		if (decorators.size() > 0)
			ast.setDecorators(decorators);
	...
	public final ASTNode visit(final Type node) throws Exception {
		if (node.getComputedName() != null) {
			return visit(node.getComputedName());
		}
		return new SimpleReference(0, 0, node.getName());
	}

	public final ASTNode visit(final Modifier node) throws Exception {
		ExpressionList sts = new ExpressionList();
		for (Expression st : node.getAnnotationValuesList()) {
			ASTNode chast = visit(st);
			if (chast != null)
				sts.addExpression((org.eclipse.dltk.ast.expressions.Expression) chast);
		}
		if (sts.getExpressions().size() > 0)
			return new PythonFunctionDecorator(new DLTKToken(0, node.getAnnotationName()), 0, 0, sts);
		return new PythonFunctionDecorator(new DLTKToken(0, node.getAnnotationName()), 0, 0);
	}
	...
	public final ASTNode visit(final Method node) throws Exception {
		...
		if (node.getReturnType() != null) {
			ASTNode chast = visit(node.getReturnType());
			if (chast != null)
				ast.setReturnType(chast);
		}
	...
	public final ASTNode visit(final Expression node) throws Exception {
		...
		if (node.getKind() == ExpressionKind.METHODCALL || node.getKind() == ExpressionKind.ARRAYACCESS
	...
	public final ASTNode visitGlobalStatement(final Statement node) throws Exception {
		if (node.getExpressionsCount() > 0)
			return new GlobalStatement(0, 0, wrapInExpressionList(node.getExpressionsList()));
		return new GlobalStatement(0, 0, new ExpressionList());
	}
	...
	public final ASTNode visitWithStatement(final Statement node) throws Exception {
		...
		for (Variable v : node.getVariableDeclarationsList()) {
			if (v.getInitializer() != null && v.getComputedName() != null) {
				...
				asExp = (org.eclipse.dltk.ast.expressions.Expression) visit(v.getComputedName());
		...
		ast = new PythonWithStatement(new DLTKToken(), withExpressions, body, 0, 0);
		return ast;
	}
	...
	public final ASTNode visitContinueStatement(final Statement node) throws Exception {
		ContinueStatement ast = new ContinueStatement(new DLTKToken(), null, new DLTKToken());
		return ast;
	}

	public final ASTNode visitBreakStatement(final Statement node) throws Exception {
		BreakStatement ast = new BreakStatement(new DLTKToken(), null, new DLTKToken());
		return ast;
	}
	...
	public final ASTNode visitIfStatement(final Statement node) throws Exception {
		...
		if (elseBlock != null)
			ast.setElse((org.eclipse.dltk.ast.statements.Statement) elseBlock);
		return ast;
	}
	...
	public final ASTNode visitForStatement(final Statement node) throws Exception {
		...
		for (Variable v : node.getVariableDeclarationsList()) {
			if (v.getComputedName() != null) {
				...
				if (chast != null) {
					org.eclipse.dltk.ast.expressions.Expression tex = (org.eclipse.dltk.ast.expressions.Expression) chast;
		...
		ast = new PythonForStatement(new DLTKToken(), mains, cond, (Block) body);
		if (elseBlock != null)
	...
	public final ASTNode visitTryFinallyStatement(final Statement node) throws Exception {
		...
		ast = new TryFinallyStatement(new DLTKToken(), body);
	...
	public final ASTNode visitExceptStatement(final Statement node) throws Exception {
		...
		if (node.getVariableDeclaration() != null) {
			if (node.getVariableDeclaration().getComputedName() != null) {
				exp = (org.eclipse.dltk.ast.expressions.Expression) visit(
						node.getVariableDeclaration().getComputedName());
			}
		}
		if (node.getExpressionsCount() >= 1) {
			ASTNode chast = visit(node.getExpressions(0));
		...
		ast = new PythonExceptStatement(new DLTKToken(), exp, msg, body);
		return ast;
	}
	...
	public final ASTNode visitTryStatement(final Statement node) throws Exception {
		...
		for (int i = 1; i < node.getStatementsCount(); i++) {
			...
			if (chast != null) {
				if (chast instanceof PythonExceptStatement || chast instanceof TryFinallyStatement)
					catches.add(chast);
				else
					elseBlock = (org.eclipse.dltk.ast.expressions.Expression) chast;
			}
		}
		ast = new PythonTryStatement(new DLTKToken(), body, catches);
		if (elseBlock != null)
			ast.setElseStatement(elseBlock);
		return ast;
	}
	...
	public final List<CallHolder> visitCallHolderInMethodCallExpression(final Expression node) throws Exception {
		List<CallHolder> calls = new ArrayList<CallHolder>();

		if (node.getMethodArgsCount() == 0)
			calls.add(new CallHolder(0, 0, new EmptyExpression()));
		else {
			for (Expression l : node.getMethodArgsList()) {
				if (l.getExpressionsCount() == 0)
					calls.add(new CallHolder(0, 0, new EmptyExpression()));
				else
					calls.add(new CallHolder(0, 0, wrapInExpressionList(l.getExpressionsList())));
			}
		}
		return calls;
	}
	...
	public ASTNode makeImportExpression(String name) {
		if (name.equals("*"))
			return new PythonAllImportExpression();
	...
	public PythonImportAsExpression makeImportASExpression(String name, String asName) {
		return new PythonImportAsExpression(new DLTKToken(0, name), new DLTKToken(0, asName));
	}
	...
	public final ASTNode visitLambdaExpression(final Expression node) throws Exception {
		...
		return new PythonLambdaExpression(new DLTKToken(), arg, exp);
	}
	...
	public final ASTNode visitIndexExpression(final Expression node) throws Exception {
		if (node.getExpressionsCount() == 0)
			...
			if (node.getExpressionsCount() > 0)
				ast.setTest((org.eclipse.dltk.ast.expressions.Expression) visit(node.getExpressions(0)));

			if (node.getExpressionsCount() > 1)
				ast.setCondition((org.eclipse.dltk.ast.expressions.Expression) visit(node.getExpressions(1)));

			if (node.getExpressionsCount() > 2)
				ast.setSlice((org.eclipse.dltk.ast.expressions.Expression) visit(node.getExpressions(2)));
	...
	public final PythonArgument visitArgument(final Variable node) throws Exception {
		...
		if (node.getVariableType() != null)
			ast.setDataType(visit(node.getVariableType()));
		return ast;
	}
	...
	public final ASTNode visitDictExpression(final Expression node) throws Exception {
		...
		for (boa.types.Ast.Expression st : node.getExpressionsList()) {
			if (st.getExpressionsCount() == 2) {
				...
				ast.putExpression((org.eclipse.dltk.ast.expressions.Expression) key,
						(org.eclipse.dltk.ast.expressions.Expression) value);
			}
		}
		return ast;
	}
	...
	public final ASTNode visitConditionalExpression(final Expression node) throws Exception {
		...
		org.eclipse.dltk.ast.expressions.Expression expThen = null;
		org.eclipse.dltk.ast.expressions.Expression expCond = null;
		org.eclipse.dltk.ast.expressions.Expression expElse = null;
		if (node.getExpressionsCount() >= 1) {
			ASTNode chast = visit(node.getExpressions(0));
			if (chast != null)
				expThen = (org.eclipse.dltk.ast.expressions.Expression) chast;
		}
		if (node.getExpressionsCount() >= 2) {
			ASTNode chast = visit(node.getExpressions(1));
			if (chast != null)
				expCond = (org.eclipse.dltk.ast.expressions.Expression) chast;
		}
		if (node.getExpressionsCount() >= 3) {
			ASTNode chast = visit(node.getExpressions(2));
			if (chast != null)
				expElse = (org.eclipse.dltk.ast.expressions.Expression) chast;
		}
		ast = new ShortHandIfExpression(expCond, expThen, expElse);
		return ast;
	}
	...
	public final ASTNode visitAssignExpression(final Expression node) throws Exception {
		...
		org.eclipse.dltk.ast.statements.Statement left = (org.eclipse.dltk.ast.statements.Statement) visit(
				node.getExpressions(0));
		org.eclipse.dltk.ast.statements.Statement right = (org.eclipse.dltk.ast.statements.Statement) visit(
				node.getExpressions(1));
	...
	public final ExpressionList wrapInExpressionList(List<Expression> l) throws Exception {
		...
		for (Expression e : l) {
			org.eclipse.dltk.ast.expressions.Expression ex = (org.eclipse.dltk.ast.expressions.Expression) visit(e);
	...
	public boolean isBinaryExpressionKind(final Expression node) {
		...
		return node.getKind().toString().startsWith("OP") || node.getKind().toString().endsWith("EQ")
				|| node.getKind().toString().endsWith("IN") || node.getKind().toString().startsWith("IS")
				|| node.getKind().toString().startsWith("LOGICAL") || node.getKind().toString().startsWith("BIT")
				|| node.getKind() == ExpressionKind.GT || node.getKind() == ExpressionKind.LT
