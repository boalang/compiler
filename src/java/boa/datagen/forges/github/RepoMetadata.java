...
public class RepoMetadata {
	...
	public boolean build() {
        ...
        if (jsonProject.has(GIT_CREATE)) {
        	...
        	this.created_timestamp = getTimeStamp(time); // project.setCreatedDate(timestamp * 1000000);
        }
	...
	private void buildProgrammingLanguages(File metadataFile, String id) {
		File file = new File(metadataFile.getParentFile().getParentFile().getAbsolutePath() + "/languages/" + id + ".json");
		if (file.exists()) {
