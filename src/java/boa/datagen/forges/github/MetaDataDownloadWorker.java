...
//	final int endFileNumber;
	...
	public MetaDataDownloadWorker(String repoPath, String output, TokenList tokenList, int i) {
		this.output = output;
	...
	public void downloadRepoMetaDataForRepoIn() {
		...
		for (int j = 0; j < size; j++) {
			...
			//System.out.println(index + " processed: " + file.getName());
		}
	...
	private void addRepo(String output, JsonObject repo) {
		...
		if (this.reposArray.size() % RECORDS_PER_FILE == 0) {
			fileToWriteJson = new File(output + "/Thread-" + index + "-page-" + Counter + ".json");
			while (fileToWriteJson.exists()) {
				System.out.println("file thread-" + index + "-page-" + Counter + " arleady exist");
				Counter++;
				fileToWriteJson = new File(output + "/Thread-" + index + "-page-" + Counter + ".json");
			}
