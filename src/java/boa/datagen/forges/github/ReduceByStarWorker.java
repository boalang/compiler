...
public class ReduceByStarWorker implements Runnable {
	...
	public ReduceByStarWorker(String repoPath, String output, TokenList tokenList, int index) {
		this.output = output;
	...
	private void addRepo(String output, JsonObject repo) {
		...
		if (this.javarepos.size() % RECORDS_PER_FILE == 0) {
			fileToWriteJson = new File(
					output + "/scala/Thread-" + Thread.currentThread().getId() + "-page-" + javaCounter + ".json");
