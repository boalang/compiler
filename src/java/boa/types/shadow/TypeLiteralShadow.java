 ...
 * @author kaushin
 */
public class TypeLiteralShadow extends boa.types.BoaShadowType  {
    /**
     * Construct a {@link TypeLiteralShadow}.
     */
    public TypeLiteralShadow() {
        super(new boa.types.proto.ExpressionProtoTuple());

        addShadow("type_lit", new boa.types.BoaString());
    }
    ...
    public Node lookupCodegen(final String name, final Factor fact, final SymbolTable env) {
        if ("type_lit".equals(name)) return ASTFactory.createSelector("string_1", new boa.types.BoaString(), env);

        throw new RuntimeException("invalid shadow field '" + name + "' in shadow type TypeLiteralShadow");
    }
    ...
    public IfStatement getManytoOne(final SymbolTable env, final Block b) {
        return getManytoOne(env, b, "istypelit", new boa.types.proto.ExpressionProtoTuple());
    }
     ...
     */
    public boa.types.Ast.Expression flattenMessage(final boa.types.Ast.Expression.TypeLiteral m) {
