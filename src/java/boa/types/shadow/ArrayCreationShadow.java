 ...
 * @author kaushin
 */
public class ArrayCreationShadow extends boa.types.BoaShadowType  {
    /**
    ...
    public ArrayCreationShadow() {
        ...
        addShadow("dimensions", new boa.types.BoaProtoList(new boa.types.proto.ExpressionProtoTuple()));
        addShadow("initializer", new boa.types.proto.ExpressionProtoTuple());
    }
    ...
    public Node lookupCodegen(final String name, final Factor fact, final SymbolTable env) {
        ...
        if ("dimensions".equals(name)) return ASTFactory.createSelector("expressions_1", new boa.types.BoaProtoList(new boa.types.BoaProtoList(new boa.types.proto.ExpressionProtoTuple())), env);
        if ("initializer".equals(name)) return ASTFactory.createSelector("expression_1", new boa.types.proto.ExpressionProtoTuple(), env);

        throw new RuntimeException("invalid shadow field '" + name + "' in shadow type ArrayCreationShadow");
    }

    /** {@inheritDoc} */
    @Override
    public Expression getKindExpression(final SymbolTable env) {
        return getKindExpression("ExpressionKind", "NEWARRAY", new boa.types.proto.enums.ExpressionKindProtoMap(), env);
    }
     ...
     */
    public boa.types.Ast.Expression flattenMessage(final boa.types.Ast.Expression.ArrayCreation m) {
        ...
        for (int i = 0; i < m.getDimensionsCount(); i++) b.addExpressions1(m.getDimensions(i));
        if (m.hasInitializer()) b.setExpression1(m.getInitializer());
        return b.build();
    }
