...
public class TypeCheckingVisitor extends AbstractVisitorNoReturn<SymbolTable> {
		...
		if (n.getOpsSize() > 0) {
			for (final Node node : n.getOps()) {
				if (node instanceof Selector) {
					...
					if (((Identifier)n.getOperand()).getToken().equals("getvalue") && formalParameters.size() == 1)
						type = lastRetType;
				}
				node.type = type;
			}
	...
	public void visit(final SimpleExpr n, final SymbolTable env) {
		...
		if (type instanceof BoaArray) { // only allow '+' (concat) on arrays
		...
		} else if (type instanceof BoaString) { // only allow '+' (concat) on strings
			for (final String s : n.getOps())
				if (!s.equals("+"))
